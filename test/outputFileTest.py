    ...
    if row[5] == "NY":
        ...
        for item in mfObj["aggregateList"]:
            ...
            match aggregate:
                ...
                case "min":
                    mfObj["groupingResultDict"][key][subKey] = min(row[targetIndex],
                                                                   mfObj["groupingResultDict"][key][subKey])

                case "max":
                    mfObj["groupingResultDict"][key][subKey] = max(row[targetIndex],
                                                                   mfObj["groupingResultDict"][key][subKey])

                case "avg":
                    mfObj["groupingResultDict"][key]["sum"] += row[targetIndex]
                    mfObj["groupingResultDict"][key]["count"] += 1
                    mfObj["groupingResultDict"][key][subKey] = mfObj["groupingResultDict"][key]["sum"] / \
                                                               mfObj["groupingResultDict"][key]["count"]
